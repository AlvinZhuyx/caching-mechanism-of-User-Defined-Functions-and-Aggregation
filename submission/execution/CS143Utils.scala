...
object CS143Utils {
  ...
  /*
  * This method get Bytes from Row
  * */
  ...
  def getUdfFromExpressions(expressions: Seq[Expression]): ScalaUdf = {
    ...
    for (item <- expressions){
      if (item.isInstanceOf[ScalaUdf]){
        Udf = item.asInstanceOf[ScalaUdf]
      }
    }
    return Udf
  }
  ...
  def apply(
    ...
    new Iterator[Row] {
      ...
      def next() = {
        ...
        if (this.hasNext()){
          var tmp = input.next()
          var tmpkey = cacheKeyProjection(tmp)
          var tmpudf: Row = null
          if (!cache.containsKey(tmpkey)){
            tmpudf = udfProject(tmp)
            cache.put(tmpkey, tmpudf)
          }
          else {
            tmpudf = cache.get(tmpkey)
          }
  ...
  def apply(resultExpressions: Seq[Expression],
    ...
    new Iterator[Row] {
      ...
      def next() = {
        ...
        if (hasNext()){
          var currentEntry = input.next()
          var currentGroup = currentEntry._1
          var currentBuffer = currentEntry._2
          var aggregateResults = new GenericMutableRow(1)
