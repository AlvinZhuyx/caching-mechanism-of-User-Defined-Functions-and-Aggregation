...
case class SpillableAggregate(
  ...
  /** Physical aggregator generated from a logical expression.  */
    // find the aggregateExpression, like what we do in UDF, if there are multiple Aggregate expression we find the last one
  private[this] val aggregator: ComputedAggregate = {
    // var tmp:AggregateExpression = null
    // var result: ComputedAggregate = null

    // for (item <- aggregateExpressions) {
      /*
      if (item.isInstanceOf[AggregateExpression]) {
        tmp = item.asInstanceOf[AggregateExpression]
        result = ComputedAggregate(
          tmp,
          BindReferences.bindReference(tmp, child.output),
          AttributeReference(s"aggResult:$tmp", tmp.dataType, tmp.nullable)())

      }
      */
    // }
      var result = aggregateExpressions.flatMap { agg =>
        agg.collect {
  ...
  private[this] val aggregatorSchema: AttributeReference = {
    aggregator.resultAttribute
  } // IMPLEMENT ME
  ...
  def generateIterator(input: Iterator[Row], memorySize: Long = 64 * 1024 * 1024, numPartitions: Int = 64): Iterator[Row] = {
    ...
    new Iterator[Row] {
      ...
      private def aggregate(): Iterator[Row] = {
        /* IMPLEMENT THIS METHOD */
        var hashiterator:Iterator[(Row, AggregateFunction)] = null
        if (groupingExpressions.isEmpty) {
          var buffer = newAggregatorInstance()
          var currentRow: Row = null
          while (data.hasNext) {
            currentRow = data.next()
            buffer.update(currentRow)
          }
         currentAggregationTable(EmptyRow) = buffer
        }
        else{
          var currentRow: Row = null
          while(data.hasNext){
            ...
            if (currentBuffer == null){
              ...
              currentAggregationTable(currentGroup) = currentBuffer
            }
            currentBuffer.update(currentRow)
          }
        }
        hashiterator = currentAggregationTable.iterator
        if (hashiterator.hasNext){
          val myAggregateIteratorGenerator = AggregateIteratorGenerator(resultExpression, Seq(aggregatorSchema) ++ namedGroups.map(_._2))(hashiterator)
          myAggregateIteratorGenerator
        }
        else {
          null
        }
